...
*  cached_network_image: кэширование изображений персонажей
...
## Исправление ошибок
* На странице избранных отображались не все избранные персонажи, а только те, которые уже были загружены на главной странице, потому что список избранных брался из state home_bloc_bloc вместо того чтобы загружать из бд. Исправлено в 0.1.5
* Замечания анализатора Dart исправлены (0.1.6)
* Netw переименован в CharactersApi (0.1.6)
* CharactersApi и DataCache теперь передаются через конструктор вместо прямого обращения в bloc (0.1.6)
* Изображения теперь кэшируются для отображения без интернета (0.1.6)
